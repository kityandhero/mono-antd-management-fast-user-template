...
class BaseSupplementAttachmentModal extends BaseDisplayModal {
  ...
  afterFileUploadSuccess = (file, data) => {
    ...
    that.setState({ file }, () => {
      showInfoMessage({
        text: '附件追加成功，即将设置附件信息',
        onClose: () => {
          that.supplementAttachment({ uploadHistoryId });
        },
      });
    });
  };
  ...
  supplement = (data) => {
    throw new Error('supplement need overrode to implement');
  };
...
export { BaseSupplementAttachmentModal };
